...
import { getFriendsByuserId } from '@/app/helper/get-friends-by-user-id';
...
const page: FC = async ({}) => {
  ...
  const friendsWithLastMessage = await Promise.all(friends.map(async(friend)=>{
    const [lastMessageRaw] = await fetchRedis('zrange',`chat:${charHrefConstructor(session.user.id,friend.id)}:messages`,-1,-1) as string[];
...
{friendsWithLastMessage.length===0?(<p className='text-sm text-zinc-500'>Nothing to show here...</p>):friendsWithLastMessage.map((friend)=>(
 <div key={friend.id} className='relative bg-zinc-50 border border-zinc-200 p-3 rounded-md'>
  <div className='absolute right-4 inset-y-0 flex items-center'>
    <ChevronRight  className='h-7 w-7 text-zinc-400'/>
    </div>
    <Link href={`/dashboard/chat/${charHrefConstructor(session.user.id,friend.id)}`} className='relative sm:flex'>
